/**
 * Recent-sessions index for AudioPilot.
 *
 * The browser is the source of truth for the full session body;
 * this route is only the small server-side mirror used by the
 * dashboard, the file manager and the search endpoint to know
 * what the user has been working on.
 *
 * Mirrors the WebPilot /api/webpilot/sessions/route.ts shape.
 */
...
import {
  listRecentSessions,
  recordRecentSession,
} from "@/lib/audiopilot/recent";

export const dynamic = "force-dynamic";

/** Lists the user's recent AudioPilot sessions. */
export async function GET(request: NextRequest) {
  ...
  const favoritesOnly =
    request.nextUrl.searchParams.get("favorites") === "1";
...
export async function POST(request: NextRequest) {
  ...
  const limit = checkRateLimit(`audio-recents:${user.id}`, 120, 60_000);
