...
export const products: Product[] = [
  ...
  },
  {
    ...
    highlights: [
      ...
      "Audio Library — recent files, favorites, search, sort, filter, duplicate, rename, delete, drag & drop upload, multi-file upload, download history",
      ...
      "Project Manager — create, open, duplicate, rename, delete, favourite, search and recent projects from the workspace rail and the Productivity quick actions",
      "Drag & drop upload — every surface that accepts audio, with a shared drop zone and a single helper",
      "Multi-file upload — library, batch and merger accept multiple files at once",
      "Download history — per-session list of every export, surfaced on the Library",
      "File naming and collision handling — every export funnels through a single safe-file-name helper",
    ...
    releaseNotes: [
      {
        ...
        date: "2026-08-02",
        changes: [
          "Project Manager: every workspace-rail capability (create, open, duplicate, rename, delete, favourite, search, recent) is surfaced in the Productivity quick actions and the Command Palette so the user can manage every AudioPilot project from one place",
          "Session recovery: when the workspace shell mounts with no open tabs, a card at the top of the main area offers to restore the most recent sessions for the current tool, with one-click open and a dismiss control",
          "Drag & drop upload: every surface that accepts audio (Player, Trimmer, Converter, Effects, Silence, Waveform Editor, Recorder, Merger, Splitter, Metadata, Batch, Library) now mounts a shared drop zone that highlights on hover and forwards dropped files to the same handler the file input uses",
          "Multi-file upload: the Library, Batch and Merger surfaces accept multiple files at once through a single file input, so the user can queue an entire session's worth of audio in one drop",
          "File naming and collision handling: every export helper (Trimmer, Effects, Merger, Converter, Batch, Export Center, Silence, Recorder) funnels through a single safeFileName helper that strips illegal characters, lowercases the extension and appends ' (n)' until the file name does not collide with an existing one",
          "Download history: the Library body now carries a per-session download history (cap 100, newest first) and the Library surface renders it as a separate card so the user can revisit every export the workspace has produced",
          "Shared hooks: useFileDrop (drag & drop with `isDragging` flag) and useKeyboardShortcuts (mod / shift / alt / key registry with `allowInInput` flag) live in `src/lib/audiopilot/hooks/` and are imported only by the surfaces that need them",
          "Shared utilities: safeFileName, formatFromFileName, sessionExportName, triggerDownload, shortUid, basename, formatBytesShort, isValidExportResult, uniquePrepend, formatTimestamp live in `src/lib/audiopilot/utils.ts` and are imported by every surface that needs them",
          "Reused the existing AudioPilot workspace shell, IndexedDB storage, autosave loop, search index, dashboard and recent-mirror for every new capability — no second workspace was created and no shared infrastructure was duplicated",
        ],
      },
      {
        version: "0.3.0",
