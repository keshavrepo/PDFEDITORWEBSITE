 ...
 *
 * Mirrors the DevPilot / SocialPilot / FinancePilot `client-storage.ts`
 * shape so a reader who knows one product knows them all.
 */
...
/** Generates a stable session id of the form `web-<kind>-<suffix>`. */
export function generateSessionId(kind: WebSessionKind): string {
  const suffix = Math.random().toString(36).slice(2, 10);
  const safeKind = (kind || "session").replace(/[^a-z0-9-]/gi, "").toLowerCase() || "session";
  return `web-${safeKind}-${suffix}`;
}

/** Generates a stable history entry id of the form `webhist-<suffix>`. */
export function generateHistoryId(): string {
  const suffix = Math.random().toString(36).slice(2, 10);
  return `webhist-${suffix}`;
}

function sessionToSummary(session: WebSession): WebSessionSummary {
  return {
    id: session.meta.id,
...
function entryToSummary(entry: WebHistoryEntry): WebSessionSummary {
  return {
    id: entry.id,
    kind: "history",
    title: entry.text.slice(0, 80) || "Untitled entry",
    category: (entry.category as WebSessionSummary["category"]) ?? "history",
...
export async function listSessionsStorage(
  ...
  const summaries = rows
    .map((row) => row.session)
    .filter((session): session is WebSession => Boolean(session))
