...
/**
 * DevPilot recent-sessions mirror.
 *
...
export const devSessions = pgTable(
  ...
  (table) => [
    index("dev_sessions_user_updated_idx").on(table.userId, table.updatedAt),
    index("dev_sessions_user_kind_idx").on(table.userId, table.kind),
  ]
);

/**
 * DevPilot developer history.
 *
 * One row per history entry, holding the small per-tool history
 * (recent and favourites) so the dashboard, the file manager and
 * the search can list what the developer has been working on
 * without round-tripping the local store.
 */
export const devHistory = pgTable(
  "dev_history",
  {
    id: varchar("id", { length: 80 }).primaryKey(),
    userId: uuid("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
  ...
  (table) => [
    index("dev_history_user_updated_idx").on(table.userId, table.updatedAt),
...
/**
 * AudioPilot recent-sessions mirror.
 *
 * The full session body lives in the browser (IndexedDB) so the audio
 * bytes are always available offline. This row is a small index the
 * server uses to list the user's recent sessions on the dashboard,
 * the file manager and the search results without round-tripping the
 * local store. Mirrors the WebPilot / DevPilot / SocialPilot /
 * FinancePilot `*Sessions` table shape so a reader who knows one
 * product knows them all.
 */
export const audioSessions = pgTable(
  "audio_sessions",
  {
    id: varchar("id", { length: 80 }).primaryKey(),
    userId: uuid("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
  ...
  (table) => [
    index("audio_sessions_user_updated_idx").on(table.userId, table.updatedAt),
    index("audio_sessions_user_kind_idx").on(table.userId, table.kind),
  ]
);

/**
 * AudioPilot per-tool history.
 *
 * One row per history entry, holding the small per-tool history
 * (recent and favourites) so the dashboard, the file manager and
 * the search can list what the user has been working on without
 * round-tripping the local store. Mirrors the WebPilot `webHistory`
 * table shape.
 */
export const audioHistory = pgTable(
  "audio_history",
  {
    id: varchar("id", { length: 80 }).primaryKey(),
    userId: uuid("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
  ...
  (table) => [
    index("audio_history_user_updated_idx").on(table.userId, table.updatedAt),
