...
/**
 * Activity panel for WebPilot.
 *
 * Reuses the platform-level `/api/activity` endpoint so the WebPilot
 * shell shows the same activity stream as the dashboard. The panel
 * filters to WebPilot events when possible, but falls back to a global
 * feed so the user always sees something useful.
 *
 * Mirrors the DevPilot `properties/activity-panel.tsx` shape.
 */
...
interface ActivityPanelProps {
  ...
  limit?: number;
  /** When true, only show WebPilot events. */
  filterWeb?: boolean;
}

export function ActivityPanel({ limit = 12, filterWeb = false }: ActivityPanelProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setEntries(filterWeb ? list.filter((e) => e.productId === "webpilot") : list);
      } catch {
