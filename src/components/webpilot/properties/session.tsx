...
/**
 * WebPilot session properties panel.
 *
 ...
 *
 * Mirrors the DevPilot `properties/session.tsx` panel.
 */
...
function summariseBody(value: unknown): string {
  ...
  if (typeof record.html === "string" && (record.html as string).trim()) {
    return `Preview · ${(record.html as string).length} bytes of HTML`;
  }
  if (typeof record.source === "string" && (record.source as string).trim()) {
    return `Source · ${(record.source as string).slice(0, 40)}${(record.source as string).length > 40 ? "…" : ""}`;
  }
