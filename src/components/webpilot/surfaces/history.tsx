...
/**
 * History surface for WebPilot.
 *
 * A single session holds the per-tool history: a list of entries
 * (recent + favourites), a tool filter and a search term. The
 * history is mirrored to the server via /api/webpilot/history and
 * the entries are kept on the session body so a future diff UI can
 * compare history snapshots.
 *
 * Mirrors the DevPilot `history.tsx` surface.
 */
...
function uniqueId(): string {
  return `webhist-${Math.random().toString(36).slice(2, 10)}`;
}
...
export function HistorySurface({ session, onChange }: HistorySurfaceProps) {
  ...
  function commit(patch: Partial<WebHistoryBody>) {
  ...
  function addEntry(text: string) {
    ...
    const entry: WebHistoryEntry = {
      id: uniqueId(),
      toolName: body.toolFilter === "all" ? "html" : body.toolFilter,
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-4">
      <Card className="p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-sm font-semibold">Web history</h2>
