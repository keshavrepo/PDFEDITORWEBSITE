...
/**
 * DevPilot history panel.
 *
 * Reuses the platform-level `/api/devpilot/history` endpoint so the
 * DevPilot shell shows the per-tool history (recent + favourites)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`/api/devpilot/history?${params.toString()}`);
